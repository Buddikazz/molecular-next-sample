...
export const fetchAverageRating = async () => {
  const response = await fetch(
    `https://api.trustpilot.com/v1/business-units/find?name=staysure.co.uk`,
    {
      headers: {
        apiKey,
      },
  ...
  const { score, numberOfReviews, links }: AverageRatingJSONResponse =
    await response.json();
  ...
  return {
    ...
    link: {
      herf: "https://uk.trustpilot.com/review/www.staysure.co.uk?utm_medium=trustbox&utm_source=Mini",
    },
  };
};
...
export const fetchLatestReviews = async (language: string, limit: number) => {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to retrieve Trustpilot reviews information: ${response.statusText}`
    );
  }
